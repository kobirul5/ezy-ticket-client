...
const ManageEvents = () => {
  ...
  const allEventsArray = Array.isArray(res?.data)
    ? res.data
    : res?.data?.data || [];
  ...
  if (isLoading)
    return <div className="text-center my-8">Loading events...</div>;
  if (isError)
    return (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/20 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
            ...
            <p>
              ...
              {new Date(selectedEvent.eventDate).toLocaleDateString()} at{" "}
